  ...
  return courseData ? (
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'> 
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='text-2xl md:text-3xl'>{courseData.courseTitle}</h1>
          ...
          <div className='flex items-center space-x-2  '>
                                      ...
                                      <li className='flex items-start gap-2 py-1' key={i}>
                                        ...
                                        <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'>
                                          ...
                                          <div className='flex gap-2'>
                                            {lecture.isPreviewFree && <p className='text-blue-500' onClick={()=>setPlayerData({videoId: lecture.lectureUrl.split("/").pop()})}>Preview</p>}
                    ...
                    <div className='py-20 text-sm md:text-default'>
                      <h3 className='text-xl font-semibold text-gray-800' >Course Desctipion</h3>
                      <p className='pt-3 md:text-base text-sm rich-text' dangerouslySetInnerHTML={{__html: courseData.courseDescription}} />
                    </div>
        </div>
        {/* right column  */}
        <div class="max-w-course-card z-10 shadow-lg rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
          {playerData ? <Youtube videoId={playerData.videoId} opts={{playerVars: {autoplay:1}}} iframeClassName='w-full aspect-video'/> : 
                <img src={courseData.courseThumbnail} alt="" className="pt-5" />
          }
         <div className='p-5'>
           <div className='flex items-center gap-2'>
            <img className="w-3.5" src={assets.time_left_clock_icon} alt="time left clock icon" />
          
            <p className="text-red-500 font-medium">5 days left at this price!</p>
          </div>
